...
class Env():
    ...
    # changing speed of robot in dependance of object minimum distance
    def step(self, action):
        ...
        if state[362] > 0.3:
            ...
            vel_cmd.linear.x = 0.15 - (0.3-state[362])/3
            # vel_cmd.linear.x = 0.15 - (0.6-state[362])/5
            self.pub_cmd_vel.publish(vel_cmd)
        return np.asarray(state), reward, done, crashes, goals, time
