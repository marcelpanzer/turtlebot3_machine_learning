...
class Env():
                ...
                if i == 0:
                    ...
                    while scan.ranges[k]==0:
                        if k == 359:
                            ...
                            # print "filter: ", scan_range

                            for i in range(len(scan_range)):
                                if scan_range[i] < 0.10:
                                    scan_range2.append(0.1)
                                else:
                            ...
                            print "min_range: ", min(scan_range)

                            print "2: ", len(scan_range2)
    ...
    def speed(self, state):
        ...
        if obstacle_min_range >= 1:
            speed = 0.15
        elif obstacle_min_range < 1 and obstacle_min_range >= 0.2:
            speed = 0.15 + ((obstacle_min_range-1)/10)
        speed_goal = speed
        if goal_distance < 0.4:
            speed_goal = 0.15 + (goal_distance - 0.4)/8
    ...
    def reset(self):
        yaw_neu = 0
        if self.initGoal:
            self.start_x, self.start_y, start_orientation_2 = self.respawn_goal.getstartPosition()
            ...
            self.start_x, self.start_y = self.goal_x_map, self.goal_y_map
            self.goal_x, self.goal_y = self.respawn_goal.getPosition()
        ...
        diff_x = self.goal_x - self.start_x
        diff_y = self.goal_y - self.start_y
